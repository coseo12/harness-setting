...
8. **429 fallback 분기 자동 매핑 (BC#4 실행 단계, Phase 3)** — step 7 의 cross-validate 호출은 `cross_validate.sh` 스크립트를 경유하며, 종료 시 **outcome JSON 파일** (`${LOG_DIR}/cross-validate-<type>-<timestamp>-outcome.json`) 을 생성한다. architect 는 이 파일을 읽어 `extends.cross_validate_outcome` 을 **자동 매핑**:
   ...
   OUTCOME_FILE=$(ls -t .claude/logs/cross-validate-*-outcome.json | head -1)
   OUTCOME=$(grep -o '"outcome": *"[^"]*"' "${OUTCOME_FILE}" | sed 's/.*"\([^"]*\)"$/\1/')
   # OUTCOME 값: "applied" | "429-fallback-claude-only" | "fatal-error"
   # extends.cross_validate_outcome 에 그대로 기록
   ```
   - `"applied"` (exit 0): 정상. 추가 박제 없음
   - `"429-fallback-claude-only"` (exit 77): 이슈 코멘트 `### 교차검증 반영` 서브섹션 첫 줄에 **`claude-only analysis completed — 단일 모델 편향 노출 미확보`** 명시 박제. 노출 효율 최대 앵커(CRITICAL 개정 / ADR 신규·중대 개정·폐기 / MINOR 이상 Behavior Changes) 해당 시 스크립트가 자동으로 reminder 이슈 dry-run 출력 (outcome JSON 의 `reminder_issue` 필드에 `"dryrun"` 또는 `"created"` 기록됨 — `REMINDER_ISSUE_DRYRUN=0` 으로 실제 생성 시)
   - `"fatal-error"` (exit 1): 비-capacity 치명적 오류 (인자 오류, 권한 등). `blocking_issues` 에 원인 기록
