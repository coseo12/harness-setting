...
```
todo → in-progress → review → audit-passed → qa → qa-passed → done
          ↑                                          │
          └──────────────────────────────────────────┘ (반려 시)
```
...
- `status:in-progress` → `status:review` : PR 생성
- `status:review` → `status:audit-passed` : Auditor 정적 분석 통과
- `status:audit-passed` → `status:qa` : Reviewer 승인
- `status:qa` → `status:qa-passed` : QA 테스트 통과
- `status:qa-passed` → `status:done` : Integrator 정합성 검증 통과
- `status:review` → `status:in-progress` : Auditor 블로커 발견 또는 Reviewer 변경 요청
