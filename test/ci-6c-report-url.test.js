...
// ============================================================
// #228 회귀 가드 3건 (PR #227 reviewer/qa non-blocking 제안)
// ============================================================

test('REPORT_ISSUE_BASE_URL 이 package.json::repository.url 과 일치 (포크/리네임 drift 가드)', () => {
  // 저장소가 포크되거나 리네임되면 하드코딩된 REPORT_ISSUE_BASE_URL 이 drift 된다.
  // npm 표준 repository.url 필드를 SSoT 로 삼아 조기 감지.
  const pkg = require('../package.json');
  assert.ok(pkg.repository && pkg.repository.url, 'package.json 에 repository.url 필수');
  const repoBase = pkg.repository.url.replace(/^git\+/, '').replace(/\.git$/, '');
  assert.strictEqual(
    REPORT_ISSUE_BASE_URL,
    `${repoBase}/issues/new`,
    'REPORT_ISSUE_BASE_URL 이 repository.url 기반 경로와 일치'
  );
});

test(
  'collectEnvMeta: ci.yml 읽기 실패 (권한 거부) → ciHash="unreadable"',
  { skip: process.platform === 'win32' ? 'win32 chmod 효과 제한' : false },
  () => {
    // 3-way fallback (missing / hex / unreadable) 중 unreadable 경로만 테스트 미커버였음.
    // chmod 000 으로 읽기 불가 상태를 만들어 fs.readFileSync 가 EACCES 로 throw 하도록 한다.
    const cwd = makeTmpCwd('unreadable-ci');
    const ciAbs = path.join(cwd, '.github', 'workflows', 'ci.yml');
    try {
      fs.mkdirSync(path.dirname(ciAbs), { recursive: true });
      fs.writeFileSync(ciAbs, 'name: CI\n');
      fs.chmodSync(ciAbs, 0o000);

      // root 권한으로 실행될 경우 (CI runner 에서 드물게) chmod 000 도 읽을 수 있어 스킵 판정.
      let readable = true;
      try {
        fs.readFileSync(ciAbs);
      } catch {
        readable = false;
      }
      if (readable) {
        return; // root/Windows WSL 등 — unreadable 경로 테스트 환경 확보 실패, silent skip
      }

      const meta = collectEnvMeta(cwd);
      assert.strictEqual(meta.ciHash, 'unreadable', '읽기 실패 시 unreadable 마커');
    } finally {
      try {
        fs.chmodSync(ciAbs, 0o644); // rmSync 가 실패하지 않도록 권한 복구
      } catch {
        // cleanup 실패는 무시
      }
      cleanup(cwd);
...
test('build6cReportUrl: 극단 긴 harnessVersion — 현 동작 (길이 팽창 허용) 영속 관찰', () => {
  // 판정: (a) 관찰만. truncation 도입 (b) 는 현재까지 실측 초과 사례 없음 — 추상 기능 제어 회피.
  // 현 동작을 영속 기록해 향후 의도치 않은 truncation 도입 시 회귀로 감지.
  // 판정 근거는 PR 본문 참조 — reviewer 제안 #3 갈래 (a)/(b) 중 (a) 선택.
  const longVersion = 'x'.repeat(10000);
  const meta = {
    harnessVersion: longVersion,
    nodeVersion: 'v20.11.1',
    osString: 'darwin-arm64',
    ciHash: 'a1b2c3d4e5f6',
  };
  ...
  // 현 동작: URL 은 여전히 생성되며, 8192 제한을 **초과**한다 (10000 글자 harnessVersion 이 body 에 원형 삽입).
  assert.ok(url.length > 8192, `현 동작: 극단 긴 메타는 URL 8KB 제한을 초과 (length=${url.length})`);

  // 생성 자체는 실패하지 않고, URL 구조는 유효 (파싱 가능).
  const parsed = new URL(url);
  assert.strictEqual(parsed.origin + parsed.pathname, REPORT_ISSUE_BASE_URL);

  // 긴 버전 문자열이 body 에 손실 없이 담긴다 (현 동작 — 미래 truncation 도입 시 이 assertion 이 변경될 것).
  const body = parsed.searchParams.get('body');
  assert.ok(body.includes(longVersion), '현 동작: 긴 harnessVersion 이 body 에 원형 삽입됨');
});
