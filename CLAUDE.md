...
- **이탈 시그널 4개** (2개 이상 충족 시 사용자 고지):
  1. 메인 오케스트레이터가 **upstream 레포에 PR 3+ 생성** (같은 세션)
  2. **릴리스 태그 증가 ≥ 프로젝트 이슈 증가** — 인프라 작업 비중이 도메인 작업을 역전
  3. 세션 시간이 **본래 작업 예상 시간의 2배 초과**
  4. 같은 세션에 **관심사 혼합** — 원 의도 + 인프라 + 교차검증 + 후속 분리 등 4+ 트랙 병렬
- **중간 대응 규약** — 시그널 발견 시:
  1. upstream PR **2개 이상 연쇄 생성 직후** 사용자에게 명시적 선택 요청: **"원 의도 (예: X 작업) 복귀"** vs **"현 작업 (예: 인프라 fix 체인) 완결"**
  2. 세션 시간 × 2 초과 시 같은 확인 루틴 트리거
...
- **예외** — 사용자가 명시적으로 "세션 전환 허용" / "인프라 집중" 모드를 선언했거나, 원 의도가 곧 인프라 작업인 경우 (예: harness 릴리스 운영 세션) 는 시그널 무시
