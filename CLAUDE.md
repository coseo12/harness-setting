...
- **메인 오케스트레이터 단계 게이트 (volt [#77](https://github.com/coseo12/volt/issues/77))** — developer sub-agent 마무리 후 reviewer/qa 단계 건너뛰고 사용자 실 Chrome 검증/머지로 직행 금지. 순서 강제: `developer → reviewer → qa → 사용자/머지`. 예외: docs only / chore (행동 변화 없음 — "행동 변화 vs 문서 변경 판정 질문" 적용). developer 가 검증한 headless pixel diff 는 자기 변경분 baseline 을 자기 변경분으로 비교하는 **self-compare 자명 PASS** 라 회귀 신호 0 — reviewer/qa 단계가 별도 환경 (`channel: 'chrome'` 강제) + 도메인 특화 가드 (sphere/billboard 판별, 색상·visibility 검증) 로 차단. volt [#24](https://github.com/coseo12/volt/issues/24) (sub-agent 박제 누락) / volt [#33](https://github.com/coseo12/volt/issues/33) (headless 한계) / volt [#72](https://github.com/coseo12/volt/issues/72) (DoD PASS ≠ 제품 동작) 의 워크플로 게이트 변형. 상세: [docs/lessons/headless-browser-verification.md](docs/lessons/headless-browser-verification.md)
