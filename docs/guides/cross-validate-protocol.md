...
>
> **Phase 1A (#269, 2026-06-18 Gemini CLI 종료 대응)**: 백엔드 도구를 gemini-cli → Antigravity `agy` 로 교체. 현재 안내 본문은 도구 중립 ("외부 검증 모델") + 현재 도구 (`agy`) 명시. 역사적 사례 (volt #23/29/40/55/51 등) 의 "Gemini" 인용은 시점 사실이라 보존.

---

## 1. 개요

정답이 없는 의사결정에서 외부 검증 모델 (현재 Antigravity `agy`, Phase 1A #269 부터 — 이전 gemini-cli) 의 두 번째 시각을 활용한다.

- 외부 모델 실패 시 스킵하고 "Claude 단독 분석" 을 명시한다
- 경량 모델 폴백은 하지 않는다 — 교차검증의 가치는 깊은 분석에 있다

## 2. API capacity 소진 폴백 프로토콜

첫 capacity 실패 (이전 gemini-cli 의 429 / 현재 agy 의 timeout/quota stderr) 시 즉시 Claude 단독으로 내려가지 말고 단계적으로 처리:

### 2.1 단계별 처리

1. `agy -p "hello"` 로 capacity 체크 후 본 검증 1회 **지연 재시도** (연속 실패는 대개 수초~수분 단위로 해소됨. 이전 gemini-cli 시점은 `gemini -p "hello"`)
2. 2차 시도도 capacity 실패/timeout 이면 Claude 단독 분석으로 전환. 단, **"claude-only analysis completed — 단일 모델 편향 노출 미확보"** 를 결과 박제에 명시 기록. 박제 위치는 컨텍스트별 1:1:
...
3. **노출 효율 최대 타이밍** 이었다면 **reminder 이슈로 박제**. 최대 타이밍은 다음 4개 앵커 중 하나에 해당할 때:
   ...
   - **프로젝트 원칙·철학 선언** — ADR 보다 추상도 높은 상위 원칙 (예: "Fact-First / Visual-Second", "Correctness-First / Performance-Second") 을 박제할 때. 단일 모델 편향이 원칙 수준에서 특히 강하게 작용 (볼트 [#55](https://github.com/coseo12/volt/issues/55) 에서 Gemini 고유 발견 6종을 원칙 선언 직후 교차검증으로 끌어낸 실증 — 시점 사실 보존). **식별 질문 3개** (모두 yes 여야 본 앵커 해당 — 일반 ADR·릴리스 Notes 와 구분):
   ...
   reminder 이슈 제목 예시 `[#<원 PR 번호>] cross-validate 재시도 — agy 응답성 복구 후`. 본문에 원 PR/ADR 링크 + 재시도 시 확인할 범주(범주 오류 / 암묵 전제 / 비목표 대조) 명시. 응답성 복구 후 close 또는 재검증 결과 반영

### 2.2 스크립트 레벨 강제 (v2.18.0~)

[.claude/skills/cross-validate/scripts/cross_validate.sh](../../.claude/skills/cross-validate/scripts/cross_validate.sh) 는 폴백 프로토콜을 하드코딩한다. capacity 실패 수신 시 `check_external_capacity()` (`agy -p "hello"`) + 지연 후 재시도 → 최종 실패 시 **stderr 에 `claude-only analysis completed — 단일 모델 편향 노출 미확보` 프리픽스 출력 + stdout 에 `[claude-only-fallback]` 헤더 + exit code 77** 반환 (v2.20.0~ stdout 헤더 대칭).

호출 측이 `CROSS_VALIDATE_ANCHOR` 환경변수 (`MINOR-behavior-change` / `ADR-new-or-amendment` / `CRITICAL-directive-revision`) 를 설정하면 **reminder 이슈 생성** (기본 dry-run, `REMINDER_ISSUE_DRYRUN=0` 으로 실제 생성). **재시도 sleep 은 지수 증가 (2^attempt × BASE)** 로 v2.20.0 부터 변경 (이전: linear attempt × BASE). `check_external_capacity()` 는 v2.20.0 부터 **capacity 부족 2 / 비-capacity probe 실패 1 / 정상 0** 으로 exit code 분리 (호출 측 로그 차별화). Phase 1A (#269) 부터 capacity 판정은 **stderr `^Error: (timed out|rate limit|quota|not logged|unauthorized|forbidden)` 패턴 매칭** — agy 는 timeout 시에도 exit 0 반환 (이전 gemini-cli 는 exit 1 + `RESOURCE_EXHAUSTED` 패턴). 스모크 테스트는 `test/cross-validate-fallback.test.js` 가 mock agy 바이너리로 각 분기 + **stateful 복구 분기(1차 capacity 실패 → 2차 정상)** 를 검증.
...
> **NOTE (Phase 1A)**: sentinel `"429-fallback-claude-only"` 명칭은 5 agents `parse-cross-validate-outcome.sh` backward-compat 위해 **유지**. agy 의 timeout/quota/not-logged 등 모든 capacity 실패가 본 sentinel 로 통합. 명칭 일반화는 별도 PR (5 agents 동시 갱신 필수).
...
`[claude-only-fallback]` stdout 헤더는 **fatal (exit 1) 경로에서도 동일하게 출력**된다. fatal vs capacity 정확 구분은 반드시 **outcome JSON 의 `outcome` 필드** (또는 `parse-cross-validate-outcome.sh` 헬퍼) 참조. stdout 헤더 단독으로 분기하는 호출 측 코드는 두 오류를 구분 못한다.
...
**교차검증 결과는 Claude가 재분석**: 외부 모델 산출물을 합의/이견/고유발견으로 분류하고, 과대 대응은 근거와 함께 반려. 맹목 수용 금지.

## 5. Claude 자체 편향 4종 셀프 체크리스트

cross-validate 호출 **전** 에 Claude 자신의 산출물을 아래 4가지 편향에 대조. 외부 모델 이견이 수용된 실측 사례에서 반복 관찰된 패턴 (volt [#55](https://github.com/coseo12/volt/issues/55) — 당시 Gemini 시점 인용, 시점 사실 보존). 셀프 감지 못하면 cross-validate 가 발견하므로 안전망이나, 편향 목록을 미리 알고 있으면 설계 단계에서 1차 필터링 가능.
...
체크리스트 통과 못 한 항목이 있으면 해당 부분을 cross-validate 호출 프롬프트에 **명시적 질문으로 삽입** ("B2 결합 감지 요청" 등) — 외부 모델이 그 축에 집중하도록 유도.

## 6. 외부 툴 동작 주장은 실측 필수 (Claude 도입 / 외부 모델 제안 공통)

외부 모델의 개선 제안뿐 아니라 **Claude 자신이 새 도구 flag 를 도입** 할 때도 동일 가드 적용. "툴이 알아서 처리한다" 류 추측성 서술은 특히 위험.
...
6. **cross-validate 호출 프롬프트에 명시 질문 삽입** — 본 PR 이 새로 도입한 외부 도구가 있으면 프롬프트에 "**도입한 `<도구>` 의 flag / 각 flag 의 공식 문서 명시 여부 / 같은 생태계 내 다른 도구의 flag 복사 여부**" 질문을 명시적으로 삽입. 외부 모델의 "침묵" 이 곧 "안전" 이 아니다 — 주장하지 않으면 가드 미발동 (volt #59)
...
2. **고유 발견의 범위 체크** — 외부 모델만의 제안이면 현재 스프린트 계약(특히 **비목표**)과 대조. 범위 내면 반영, 범위 밖(비목표와 상충)이면 **후속 이슈로 분리**. 판단 질문: "이 변경이 현재 PR 의 `Behavior Changes` 에 원 완료 기준과 직교하는 항목을 추가하는가?"
3. **분리 시 박제 규칙** — 후속 이슈를 **즉시 생성**해 맥락 유실 방지. 본문에 외부 모델 설계 스케치 인용 + `Builds on: #원PR` 링크 + 우선순위 초안(high / medium / low) 명시

**금지**: 스프린트 비목표를 "외부 모델 제안이 타당하다"는 이유만으로 무시 (CRITICAL #6 침범). 근본 해결책이라도 현재 스프린트 범위 밖이면 분리.

**근거**: volt [#23](https://github.com/coseo12/volt/issues/23), volt [#29](https://github.com/coseo12/volt/issues/29) — harness #89 (post-apply 게이트) 교차검증에서 Gemini 가 `previousSha256` 스키마 확장을 제안했고, 비목표 "매니페스트 스키마 변경 없음"과 상충하여 후속 이슈 #92 로 분리. 결과적으로 3 PR / 3 릴리스로 자연 분할되어 각 단계 위험 독립 (시점 사실 보존).
