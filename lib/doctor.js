 ...
 *
 * 분류:
 *   - main 이 develop 보다 앞섬 (mainAhead > 0) → warn: hotfix merge-back 누락 또는 release squash 미동기화 의심
 *   - main ≤ develop (mainAhead === 0) → pass: develop 이 앞서면 릴리스 대기 정상, 동일하면 릴리스 직후
 *
 * 과거 drift 사례 (v2.12.0 이전): dual PR 변형 + 고빈도 작업 압박으로 develop 이 main 보다 56 커밋 뒤처짐.
 * 이 함수가 해당 상태를 조기 탐지하는 신호원이다.
 */
function classifyGitflowDrift(mainAhead, devAhead) {
  if (mainAhead === 0) {
    return {
      status: 'pass',
      detail:
  ...
  if (branch) {
    ...
    if (hasOrigin) {
      ...
      if (!mainRef || !devRef) {
        ...
        const mainAhead = parseInt(mainAheadRaw || '0', 10);
        const devAhead = parseInt(devAheadRaw || '0', 10);
        const result = classifyGitflowDrift(mainAhead, devAhead);
